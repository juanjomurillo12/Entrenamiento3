...
r= {i:plp.LpVariable(f"r_{i}",lowBound=0,upBound=None,cat=plp.LpContinuous) for i in range(1,6)}
# -----------------------------
# Función Objetivo
# -----------------------------
...
# Restricciones
# -----------------------------
prob += 7*w[1] + w[3] - w[4] - r[1] == 17
prob += 2*w[2] + w[3] - r[2] == 30
prob += 15*w[1] + 5*w[2] - 2*w[4] - r[3] == -4
prob += 3*w[2] + 3*w[5] - r[4] == 5
prob += -w[4] + w[5] - r[5] == -10
...
for i in range(1,6):
    ...
    print(f"r_{i} = {round(plp.value(r[i]),2)}")
