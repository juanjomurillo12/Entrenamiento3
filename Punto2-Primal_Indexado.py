...
# ---------------------
prob= lp.LpProblem("Distribucion_de_helados",sense = lp.LpMinimize)

# -----------------------------
# Variables de Decisión
# -----------------------------
# Variable que indica la cantidad de helado j que se compra a la sucursal i
x = {(i,j):lp.LpVariable(f'compra_{i}_{j}',lowBound=0, cat=lp.LpContinuous)for i in S for j in H}

# -----------------------------
# Función objetivo
# -----------------------------
prob+= lp.lpSum(c[i,j]*x[i,j] for i in S for j in H)

# -----------------------------
# Restricciones
# -----------------------------

# Se debe cumplir con el requerimiento mínimo de helado j
for j in H:
    prob+= lp.lpSum(x[i,j] for i in S) >= r[j]

# No se puede comprar más helado del que la sucursal i puede vender
for i in S:
    prob+= lp.lpSum(x[i,j] for j in H) <= m[i]
...
print("Costo total de distribución de helados: ", lp.value(prob.objective))
print("Cantidad de helado que se compra a cada sucursal:")
for i in S:
    for j in H:
        print(f"Compra_{i}_{j} = {x[i,j].varValue}")
